function [B]=Alternate(S,GL,r)  %S is the matrix of skewnesses, which we obtain from PartSkew function
    %GL is a row vector of the original numbers of the genes in the initial
    %list
    display('hello');
    B=zeros(1,3,1000); %1st number (1) indicates layer, or iteration
    ...
    for i=1:250
        ...
        %B(1,3,:)
        g=G(S,B(i,2,:),B(i,3,:),r);
        B(i+1,1,1:size(g,2))=g;
        if (i>1 && isequal(B(i,1,:),B(i-1,1,:)))
            It=i-1;
            break;
        end
    end
